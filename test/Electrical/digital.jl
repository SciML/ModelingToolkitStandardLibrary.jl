...
@testset "Xor gate" begin
    ...
    for one in [set1, reset1], two in [set2, reset2]
        ...
        sol = solve(prob, Rosenbrock23())
        @test sol.retcode == :Success
        @test sol[xor.y.val] == _or.(_and.(sol[one.d.val], _not.(sol[two.d.val])),
                                          _and.(sol[two.d.val], _not.(sol[one.d.val])))

        xnor_eqs = [
            connect(one.d, xnor.x1)
...
@testset "Full Adder" begin
    ...
    for one in [set1, reset1], two in [set2, reset2], three in [set3, reset3]
        ...
        @test sol.retcode == :Success
        @test sol[fa.y0.val] == sol[fa.sum] == _xor(sol[one.d.val], sol[two.d.val], sol[three.d.val])
    end
end
...
@testset "Multiplexers" begin
    ...
    for idx1 in 1:2, idx0 in 1:2
        ...
        @named mux_model = ODESystem(mux_eqs, t, 
                                    systems=[mux4x1, out[1], select0[idx0], select1[idx1],
...
@testset "Demultiplexers" begin
    ...
    for idx1 in 1:2, idx0 in 1:2
        ...
        @named demux_model = ODESystem(eqs, t, 
                                    systems=[demux1x4, select0[idx0], select1[idx1], input,
...
@testset "Encoder and Decoder" begin
    ...
    @test sol.retcode == :Success
    @test sol[dec2x4.y0.val] == _and.(_not.(sol[input[2].d.val]),
...
@testset "Sources and DigitalPin" begin
    ...
    for α in [setₐ, resetₐ], β in [setᵦ, resetᵦ]
        ...
        @test sol.retcode == :Success
        @test ModelingToolkitStandardLibrary._and.(sol[and.x1.val], sol[and.x2.val]) == sol[and.y.val]
    end
